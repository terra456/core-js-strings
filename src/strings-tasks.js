...
function isString(value) {
  return (
    typeof value === 'string' ||
    (!!value && typeof value.valueOf() === 'string')
  );
}
...
function removeFirstOccurrences(str, value) {
  ...
  return index >= 0
    ? str.slice(0, index) + str.slice(-(str.length - index - value.length))
    : str;
}
...
function removeLastOccurrences(/* str, value */) {
  ...
  // return index >= 0
  //   ? str.slice(0, index) +
  //       str.slice(-(str.length - index - value.length), -(str.length - 1))
  //   : str;
}
 ...
 *
 * @param {number} seconds - The number of minutes (non-negative integer).
...
function isPalindrome(str) {
  return (
    str.localeCompare(str.split('').reverse().join(''), undefined, {
      sensitivity: 'accent',
...
function encodeToRot13(str) {
  ...
  return str
    ...
    .map((el) => {
      ...
      if (i >= 0) {
        return i + 13 >= a.length ? a[i + 13 - a.length] : a[i + 13];
      }
      const I = A.indexOf(el);
      if (I >= 0) {
        return I + 13 >= A.length ? A[I + 13 - A.length] : A[I + 13];
